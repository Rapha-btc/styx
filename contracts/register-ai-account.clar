...
(define-map account-proposals <ai-account> uint) ;; to find a proposal id easily
...
;; Final registry: owner → ai account
...
(define-public (propose-ai-account-registration
  ...
  (let ((proposal-id (var-get next-proposal-id)))
    ...
    (asserts! (is-none (map-get? account-proposals ai-account)) ERR_ACCOUNT_ALREADY_REGISTERED) ;; one registration per ai-account forever
      ...
      (map-set account-proposals ai-account proposal-id)
      ...
      (print {
        ...
        ai-account: (contract-of ai-account),
        owner: owner,
        agent: agent,
        proposer: tx-sender
      })
...
(define-read-only (get-owners-by-ai-account (ai-account <ai-account>))
  (map-get? ai-account-owners (contract-of ai-account))
)

;; For off chain attestator to get proposal-id from ai-account
(define-read-only (get-proposal-id-by-ai-account (ai-account <ai-account>))
  (map-get? account-proposals (contract-of ai-account))
)
